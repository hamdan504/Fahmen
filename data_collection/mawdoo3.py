...
# Organize data into rows for CSV output
rows = []
for class_name, quotes in data.items():
    good_or_bad = 1 if "Good Quotes" in class_name else 0  # Assign 1 for good, 0 for bad
    for quote in quotes:
        rows.append([quote.strip(), class_name.split(' (')[0].strip(), good_or_bad])

# Save the organized data to a CSV file
csv_file = 'organized_arabic_proverbs.csv'
with open(csv_file, 'w', encoding='utf-8', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["quote", "class", "good_or_bad"])
    writer.writerows(rows)

print(f"Organized data saved to '{csv_file}'.")
